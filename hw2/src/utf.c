...
int transcribe(int infile, int outfile) {     // need to add BOM to start of file
  int ret = 0;
  ...
  int tr_bom = program_state->encoding_to;
  if (tr_bom != UTF8) {
    #if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
    reverse_bytes(&tr_bom, 2);
    #endif
      write_to_bigendian(outfile, &tr_bom, 2);

  } else {
  write_to_bigendian(outfile, &tr_bom, 3);
}
